...
public class Organization implements Serializable {
	...
	public void rule5(){
		...
		ArrayList<String> orgSmallSet=ConfigInf.orgSmallSet;
		ArrayList<String> orgFullSet=ConfigInf.orgFullSet;		
		for(int i=0;i<orgSmallSet.size();i++){ 			
			
			String orgSmall=orgSmallSet.get(i);
			if(orgName.contains(orgSmall)){
				
				int index=orgSmallSet.indexOf(orgSmall);
				String orgFull=orgFullSet.get(index);
				orgName=orgName.replaceAll(orgSmall, orgFull);	
				break;
			}
		}		
	}	
	...
	public void rule6(){		
		...
		orgName=orgName.replaceAll("）", ")");
   		if(orgName.contains("(")){
			//inf : 单位名称	陕西省科技资源统筹中心（陕西省生产力促进中心
			if(orgName.contains(")")){				
	...
	public String handleOrg() {
		//总体思路：向汉字靠拢 ：0/o/〇规范为"O"（综合考虑规范成"O"最合适）
		rule1();
		rule2();
		rule3();
		rule4();
